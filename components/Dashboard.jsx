...
export default function Dashboard({ user }) {
  ...
  const [success, setSuccess] = useState(''); // Added success state for feedback
  ...
  const handleEdit = (type, entry, status) => {
    ...
    setEditData({
      ...
      controlled_facility: entry.controlled_facility === 1, // Convert 1/0 to boolean
    });
  ...
  const handleEditSubmit = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setLandfillWaste(landfill);
        setDivertedWaste(diverted);
        setSuccess('Waste entry updated successfully!'); // Success message for edit
  ...
  const handleApprove = async (type, id) => {
    try {
      ...
      setLandfillWaste(landfill);
      setDivertedWaste(diverted);
      setSuccess('Entry approved successfully!'); // Success message for approve
    } catch (err) {
  ...
  const handleDelete = async (type, id, status) => {
    try {
      ...
      if (response.ok) {
        ...
        setLandfillWaste(landfill);
        setDivertedWaste(diverted);
        setSuccess('Entry deleted successfully!'); // Success message for delete
      } else {
          ...
          <Col md={12}>
            <Card className="shadow-lg">
              <Card.Body>
                ...
                <p>
                  <strong>Proportion of Waste Ending in Landfill:</strong> {landfillPercentage}%
                </p>
